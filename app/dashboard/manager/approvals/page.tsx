...
import { Check, X, RotateCcw, CheckSquare, Sparkles, Loader2, ChevronDown, ChevronUp } from "lucide-react";
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-4 flex-wrap">
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {/* AI quality badge — shows score once analyzed */}
                            {aiResults[goal.id as string]?.result && (
                              <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold tabular-nums ${
                                aiResults[goal.id as string].result!.overall_score >= 8 ? "bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400"
                                : aiResults[goal.id as string].result!.overall_score >= 6 ? "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400"
                                : "bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400"}`}>
                                AI {aiResults[goal.id as string].result!.overall_score}/10
                              </span>
                            )}
                          </div>
                          {!!goal.description && (
                      ...
                      <div className="mt-3 border border-purple-100 dark:border-purple-900/50 rounded-lg overflow-hidden">
                        <button
                          ...
                          className="w-full flex items-center justify-between px-3 py-2 bg-purple-50 dark:bg-purple-950/30 hover:bg-purple-100 dark:hover:bg-purple-900/40 transition-colors text-xs font-semibold text-purple-700 dark:text-purple-300"
                        >
